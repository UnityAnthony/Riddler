...
public class RunJets : MonoBehaviour
{
    ...
    Unity.InferenceEngine.Worker engine;
    ...
    void LoadModel()
    {
        var model = Unity.InferenceEngine.ModelLoader.Load(Path.Join(Application.streamingAssetsPath ,"jets-text-to-speech.sentis"));
        engine = new Unity.InferenceEngine.Worker(model, Unity.InferenceEngine.BackendType.GPUCompute);
    }
    ...
    {      
        ...
        using var input = new Unity.InferenceEngine.Tensor<int>(new Unity.InferenceEngine.TensorShape(tokens.Length), tokens);
        engine.Schedule(input);

        var output = engine.PeekOutput("wav") as Unity.InferenceEngine.Tensor<float>;
